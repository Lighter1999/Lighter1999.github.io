...
function animateSlides() {
    ...
    sliders.forEach((slide, index, slides) => {
        ...
        slideTl.fromTo(revealImg, { x: '0%'}, { x: '100%'}); // se duce imaginea de la stanga la dreapta
        slideTl.fromTo(img, {scale: 2}, {scale: 1}, '-=1'); // -=1 face sa se miste in acelasi timp cu animatia de sus, delay
  ...
  transitions: [
      {
          ...
          enter({current, next}) {
            let done = this.async();
            const tl = gsap.timeline({defaults: {ease: 'power2.inOut'}});
            tl.fromTo('.swipe', 1, {x: '0%'}, {x: '100%', stagger: 0.1, onComplete: done});
